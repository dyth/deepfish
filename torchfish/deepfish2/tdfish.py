...
class TDSearcher:
    ...
    def search_white(self, pos, secs=None):
        ...
        if nextPosition == None:
            ...
            base_score = forward_pass(self.network, nextPosition.board)
            return nextPosition._replace(score = base_score)
...
def self_play(searchers):
    ...
    pos._replace(score = forward_pass(searchers[0].network, pos.board))
    boards = [pos]
...
if __name__ == "__main__":
    searchers = (TDSearcher(), RandomSearcher())
    weightsNum = 0
    while True:
