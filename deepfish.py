...
class Searcher:
    ...
    def bound(self, pos, gamma, depth, root=True):
        ...
        # Depth <= 0 is QSearch. Here any position is searched as deeply as is
        # needed for calmness, and so there is no reason to keep different
        # depths in the transposition table.
        depth = max(depth, 0)
        ...
        def moves():
            ...
            # Then killer move. We search it twice, but the tp will fix things
            # for us. Note, we don't have to check for legality, since we've
            # already done it before. Also note that in QS the killer must be a
            # capture, otherwise we will be non deterministic.
        ...
        # Thus, if best < gamma and best < 0 we need to double check what we
        # are doing.
    ...
    def _search(self, pos):
        ...
        # In finished games, we could potentially go far enough to cause a
        # recursion limit exception. Hence we bound the ply.
        for depth in range(1, 1000):
            self.depth = depth
            # The inner loop is a binary search on the score of the position.
            # Inv: lower <= score <= upper
            # 'while lower != upper' would work, but play tests show a margin
            # of 20 plays better.
            ...
            while lower < upper - EVAL_ROUGHNESS:
                ...
                if score < gamma:
                    upper = score
            # We want to make sure the move to play hasn't been kicked out of
            # the table, so we make another call that must always fail high and
            # thus produce a move.
    ...
    def search(self, pos, secs):
        ...
        score = self.tp_score.get((pos, self.depth, True)).lower
        return move, score
    ...
    def evalNet_search(self, pos, network):
        ...
        for m in moves:
            ...
            if self.check_valid_move(newPos):
                print(newPos.board)
                score = forward_pass(network, newPos.board)
                if score > base:
                    base, move = score, m
        return move
...
def main():
    ...
    while True:
        ...
        move = searcher.evalNet_search(pos, network)
        if pos.move(move).score == MATE_UPPER:
            print("Checkmate!")
